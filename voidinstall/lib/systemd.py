...
class Boot:
    ...
    def __exit__(self, *args, **kwargs):
        ...
        if len(args) >= 2 and args[1]:
            ...
            log(
                f"The error above occured in a temporary boot-up of the installation {self.instance}", level=logging.ERROR, fg="red")

        shutdown = SysCommand(
            f'systemd-run --machine={self.container_name} --pty /bin/bash -c "shutdown now"')
        while self.session.is_alive():
            time.sleep(0.25)
