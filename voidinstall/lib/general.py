...
def generate_password(length=64):
    # digits, ascii_letters, punctiation (!"#$[] etc) and whitespace
    haystack = string.printable
...
class SysCommandWorker:
    ...
    def __contains__(self, key: bytes):
        ...
        if (contains := key in self._trace_log[self._trace_log_pos:]):
            self._trace_log_pos += self._trace_log[self._trace_log_pos:].find(
                key) + len(key)
        ...
        if not self.pid:
            try:
                ...
                os.execve(self.cmd[0], self.cmd, {
                          **os.environ, **self.environment_vars})
...
class SysCommand:
    ...
    def create_session(self):
        ...
        self.session = SysCommandWorker(self.cmd, callbacks=self._callbacks,
                                        peak_output=self.peak_output, environment_vars=self.environment_vars)
...
def prerequisite_check():
    if not os.path.isdir("/sys/firmware/efi"):
        raise RequirementError(
            "Archinstall only supports machines in UEFI mode.")
...
def run_custom_user_commands(commands, installation):
    for index, command in enumerate(commands):
        ...
        execution_output = SysCommand(
            f"arch-chroot {installation.target} bash /var/tmp/user-command.{index}.sh")
