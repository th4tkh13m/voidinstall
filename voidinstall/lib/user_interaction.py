...
def print_large_list(options, padding=5, margin_bottom=0, separator=': '):
    ...
    spaces_without_option = longest_line - \
        (len(separator) + highest_index_number_length)
...
def generic_multi_select(options, text="Select one or more of the options above (leave blank to continue): ", sort=True, default=None, allow_empty=False):
    ...
    if type(options) not in [list, dict, type({}.keys()), type({}.values())]:
        log(
            f" * Generic multi-select doesn't support ({type(options)}) as type of options * ", fg='red')
        log(" * If problem persists, please create an issue on https://github.com/archlinux/archinstall/issues * ", fg='yellow')
        raise RequirementError(
            "generic_multi_select() requires list or dictionary as options.")
    if not options:
        log(" * Generic multi-select didn't find any options to choose from * ", fg='red')
        log(" * If problem persists, please create an issue on https://github.com/archlinux/archinstall/issues * ", fg='yellow')
        raise RequirementError(
            'generic_multi_select() requires at least one option to proceed.')
...
def ask_for_a_timezone():
    while True:
        timezone = input(
            'Enter a valid timezone (examples: Europe/Stockholm, US/Eastern) or press enter to use UTC: ').strip().strip('*.')
        if timezone == '':
            timezone = 'UTC'
...
def ask_for_bootloader(advanced_options=False) -> str:
    ...
    if has_uefi():
        if not advanced_options:
            ...
            selection = generic_select(
                choices, f'Choose a bootloader or leave blank to use systemd-boot: ', options_output=True)
            if selection != "":
...
def ask_for_audio_selection(desktop=True):
    ...
    choices = ['pipewire', 'pulseaudio'] if desktop else [
        'pipewire', 'pulseaudio', 'none']
    selection = generic_select(
        choices, f'Choose an audio server or leave blank to use {audio}: ', options_output=True)
    if selection != "":
        audio = selection
...
def ask_to_configure_network():
    ...
    if nic and nic != 'Copy ISO network configuration to installation':
        ...
        mode = generic_select(
            ['DHCP', 'IP'], f"Select which mode to configure for {nic} or leave blank for DHCP: ", options_output=False)
        if mode == 'IP':
            while 1:
                ip = input(
...
def ask_for_disk_layout():
    ...
    value = generic_select(
        options, "Found partitions on the selected drive, (select by number) what you want to do: ", allow_empty_input=False, sort=True)
...
def ask_for_main_filesystem_format(advanced_options=False):
    ...
    value = generic_select(
        options, "Select which filesystem your main partition should use (by number or name): ", allow_empty_input=False)
    return next((key for key, val in options.items() if val == value), None)
...
def manage_new_and_existing_partitions(block_device: BlockDevice) -> dict:
    ...
    while True:
        modes = [
            ...
            "Clear/Delete all partitions" if len(
                block_device_struct) else "",
            "Assign mount-point for a partition" if len(
                block_device_struct) else "",
            "Mark/Unmark a partition to be formatted (wipes data)" if len(
                block_device_struct) else "",
            "Mark/Unmark a partition as encrypted" if len(
                block_device_struct) else "",
            "Mark/Unmark a partition as bootable (automatic for /boot)" if len(
                block_device_struct) else "",
            "Set desired filesystem for a partition" if len(
                block_device_struct) else "",
        ]
        ...
        if task == 'Create a new partition':
            ...
            fstype = input(
                "Enter a desired filesystem type for the partition: ").strip()

            start = input(
                f"Enter the start sector (percentage or block number, default: {block_device.first_free_sector}): ").strip()
            if not start.strip():
                ...
                f"Enter the end sector of the partition (percentage or block number, ex: {end_suggested}): ").strip()
            if not end.strip():
                end = end_suggested
            ...
            if valid_parted_position(start) and valid_parted_position(end) and valid_fs_type(fstype):
                ...
                block_device_struct["partitions"].append({
                    # Strictly only allowed under MSDOS, but GPT accepts it so it's "safe" to inject
                    "type": "primary",
                    "start": start,
                    "size": end,
                    "mountpoint": None,
            ...
            block_device_struct.update(
                suggest_single_disk_layout(block_device)[block_device.path])
        elif task is None:
            ...
            if task == "Delete a partition":
                if (partition := generic_select(block_device_struct["partitions"], 'Select which partition to delete: ', options_output=False)):
                    del(block_device_struct["partitions"][block_device_struct["partitions"].index(
                        partition)])
                ...
                if (partition := generic_select(block_device_struct["partitions"], 'Select which partition to mount where: ', options_output=False)):
                    ...
                    mountpoint = input(
                        'Select where to mount partition (leave blank to remove mountpoint): ').strip()

                    if len(mountpoint):
                        block_device_struct["partitions"][block_device_struct["partitions"].index(
                    ...
                    if block_device_struct["partitions"][block_device_struct["partitions"].index(partition)].get('filesystem', {}).get('format', 'crypto_LUKS') == 'crypto_LUKS':
                        ...
                        while True:
                            ...
                            if not valid_fs_type(fstype):
                                log(f"Desired filesystem {fstype} is not a valid filesystem.",
                                    level=logging.ERROR, fg="red")
                                continue
                            break

                        block_device_struct["partitions"][block_device_struct["partitions"].index(
                            partition)]['filesystem']['format'] = fstype

                    # Negate the current wipe marking
                    block_device_struct["partitions"][block_device_struct["partitions"].index(
                        partition)]['format'] = not block_device_struct["partitions"][block_device_struct["partitions"].index(partition)].get('format', False)
                ...
                if (partition := generic_select(block_device_struct["partitions"], 'Select which partition to mark as encrypted: ', options_output=False)):
                    ...
                    block_device_struct["partitions"][block_device_struct["partitions"].index(
                        partition)]['encrypted'] = not block_device_struct["partitions"][block_device_struct["partitions"].index(partition)].get('encrypted', False)
                ...
                if (partition := generic_select(block_device_struct["partitions"], 'Select which partition to mark as bootable: ', options_output=False)):
                    block_device_struct["partitions"][block_device_struct["partitions"].index(
                        partition)]['boot'] = not block_device_struct["partitions"][block_device_struct["partitions"].index(partition)].get('boot', False)

            elif task == "Set desired filesystem for a partition":
                if not block_device_struct["partitions"]:
                    log("No partitions found. Create some partitions first",
                        level=logging.WARNING, fg='yellow')
                    continue
...
def select_disk_layout(block_devices: list, advanced_options=False):
    ...
    mode = generic_select(
        modes, input_text=f"Select what you wish to do with the selected block devices: ")
...
def select_disk(dict_o_disks):
    ...
    if len(drives) >= 1:
        ...
        drive = generic_select(
            drives, 'Select one of the above disks (by name or number) or leave blank to use /mnt: ', options_output=False)
        if not drive:
            return drive
...
def select_profile():
    ...
    :rtype: str
    """
    shown_profiles = sorted(
        list(list_profiles(filter_top_level_profiles=True)))
    actual_profiles_raw = shown_profiles + \
        sorted([profile for profile in list_profiles()
               if profile not in shown_profiles])
    ...
    if len(shown_profiles) >= 1:
        ...
        selected_profile = generic_select(
            actual_profiles_raw, 'Enter a pre-programmed profile name if you want to install one: ', options_output=False)
        if selected_profile:
...
def select_language(options, show_only_country_codes=True, input_text='Select one of the above keyboard languages (by number or full name): '):
    ...
    if len(languages) >= 1:
        ...
        print(
            f" -- Also, you can enter '?' or 'help' to search for more languages, or skip to use {default_keyboard_language} layout --")
...
def select_mirror_regions(mirrors, show_top_mirrors=True):
    ...
    if len(regions) >= 1:
        ...
        selected_mirror = generic_select(
            regions, 'Select one of the above regions to download packages from (by number or full name): ', options_output=False)
...
def select_driver(options=AVAILABLE_GFX_DRIVERS):
    ...
    if drivers:
        ...
        if not arguments.get('gfx_driver', None):
            arguments['gfx_driver'] = generic_select(
                drivers, input_text="Select a graphics driver or leave blank to install all open-source drivers: ")
...
def select_kernel(options):
    ...
    raise RequirementError(
        "Selecting kernels require a least one kernel to be given as an option.")
