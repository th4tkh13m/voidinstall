...
for plugin_definition in metadata.entry_points().get('archinstall.plugin', []):
    ...
    try:
        ...
        log(
            f"The above error was detected when loading the plugin: {plugin_definition}", fg="red", level=logging.ERROR)
...
# -> module (not sure how to write that in type definitions)
def import_via_path(path: str, namespace=None):
    if not namespace:
    ...
    try:
        ...
        log(err, level=logging.ERROR)
        log(
            f"The above error was detected when loading the plugin: {path}", fg="red", level=logging.ERROR)
    ...
    if namespace in sys.modules:
        ...
        if hasattr(sys.modules[namespace], 'Plugin'):
            try:
                ...
                log(err, level=logging.ERROR)
                log(
                    f"The above error was detected when initiating the plugin: {path}", fg="red", level=logging.ERROR)
        else:
            log(f"Plugin '{path}' is missing a valid entry-point or is corrupt.",
                fg="yellow", level=logging.WARNING)
