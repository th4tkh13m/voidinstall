...
class BlockDevice:
    ...
    def device(self):
        ...
        if self.info['type'] in ['disk', 'loop']:
            ...
            log(
                f"Unknown blockdevice type for {self.path}: {self.info['type']}", level=logging.DEBUG)
    ...
    def partitions(self):
        ...
        if not result[:1] == b'{':
            raise DiskError('Error getting JSON output from:',
                            f'/usr/bin/lsblk -J {self.path}')
                ...
                if part_id not in self.part_cache:
                    ...
                    if part_id not in self.part_cache or self.part_cache[part_id].size != part['size']:
                        self.part_cache[part_id] = Partition(
                            root_path + part_id, self, part_id=part_id)
    ...
    def size(self):
        ...
        output = json.loads(SysCommand(
            f"lsblk --json -b -o+SIZE {self.path}").decode('UTF-8'))
    ...
    def bus_type(self):
        output = json.loads(SysCommand(
            f"lsblk --json -o+ROTA,TRAN {self.path}").decode('UTF-8'))
    ...
    def get_partition(self, uuid):
        ...
        while count < 5:
            for partition_uuid, partition in self.partitions.items():
                ...
                log(f"uuid {uuid} not found. Waiting for {count +1} time",
                    level=logging.DEBUG)
                time.sleep(float(storage['arguments'].get('disk-sleep', 0.2)))
                count += 1
        else:
            log(f"Could not find {uuid} in disk after 5 retries",
                level=logging.INFO)
            print(f"Cache: {self.part_cache}")
            print(f"Partitions: {self.partitions.items()}")
            print(f"UUID: {[uuid]}")
            raise DiskError(
                f"New partition {uuid} never showed up after adding new partition on {self}")
