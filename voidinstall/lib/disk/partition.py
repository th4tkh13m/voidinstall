...
class Partition:
    def __init__(self, path: str, block_device: BlockDevice, part_id=None, filesystem=None, mountpoint=None, encrypted=False, autodetect_filesystem=True):
        ...
        if self.mountpoint != mount_information.get('target', None) and mountpoint:
            raise DiskError(
                f"{self} was given a mountpoint but the actual mountpoint differs: {mount_information.get('target', None)}")
    ...
    def __lt__(self, left_comparitor):
        ...
        # Not quite sure the order here is correct. But /dev/nvme0n1p1 comes before /dev/nvme0n1p5 so seems correct.
    ...
    def sector_size(self):
        output = json.loads(SysCommand(
            f"lsblk --json -o+LOG-SEC {self.path}").decode('UTF-8'))
    ...
    def start(self):
        output = json.loads(SysCommand(
            f"sfdisk --json {self.block_device.path}").decode('UTF-8'))
    ...
    def partition_type(self):
        lsblk = json.loads(SysCommand(
            f"lsblk --json -o+PTTYPE {self.path}").decode('UTF-8'))
    ...
    def uuid(self) -> Optional[str]:
        ...
        raise DiskError(
            f"Could not get PARTUUID for {self.path} using 'lsblk -J -o+PARTUUID {self.path}'")
    ...
    def detect_inner_filesystem(self, password):
        log(
            f'Trying to detect inner filesystem format on {self} (This might take a while)', level=logging.INFO)
    ...
    def has_content(self):
        ...
        temporary_mountpoint = '/tmp/' + \
            hashlib.md5(bytes(f"{time.time()}", 'UTF-8') +
                        os.urandom(12)).hexdigest()
        ...
        if (handle := SysCommand(f'/usr/bin/mount {self.path} {temporary_mountpoint}')).exit_code != 0:
            raise DiskError(
                f'Could not mount and check for content on {self.path} because: {b"".join(handle)}')
    ...
    def format(self, filesystem=None, path=None, log_formatting=True, options=[]):
        ...
        if filesystem == 'btrfs':
            ...
            if 'UUID:' not in (mkfs := SysCommand(f"/usr/bin/mkfs.btrfs {' '.join(options)} {path}").decode('UTF-8')):
                raise DiskError(
                    f'Could not format {path} with {filesystem} because: {mkfs}')
            self.filesystem = filesystem
            ...
            mkfs = SysCommand(
                f"/usr/bin/mkfs.vfat {' '.join(options)} {path}").decode('UTF-8')
            ...
            if (handle := SysCommand(f"/usr/bin/mkfs.ext4 {' '.join(options)} {path}")).exit_code != 0:
                raise DiskError(
                    f"Could not format {path} with {filesystem} because: {handle.decode('UTF-8')}")
            self.filesystem = filesystem
            ...
            if (handle := SysCommand(f"/usr/bin/mkfs.ext2 {' '.join(options)} {path}")).exit_code != 0:
                raise DiskError(
                    f'Could not format {path} with {filesystem} because: {b"".join(handle)}')
    ...
    def mount(self, target, fs=None, options=''):
        if not self.mountpoint:
            ...
            try:
                if options:
                    mnt_handle = SysCommand(
                        f"/usr/bin/mount -t {fs_type} -o {options} {self.path} {target}")
                else:
                    mnt_handle = SysCommand(
                        f"/usr/bin/mount -t {fs_type} {self.path} {target}")

                # TODO: Should be redundant to check for exit_code
                if mnt_handle.exit_code != 0:
                    raise DiskError(
                        f"Could not mount {self.path} to {target} using options {options}")
