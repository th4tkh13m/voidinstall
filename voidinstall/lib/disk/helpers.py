        ...
        if os.path.exists('/sys/bus/{}'.format(bus)):
            for device_bus in os.listdir('/sys/bus/{}/devices'.format(bus)):
                device_link = ROOT_DIR_PATTERN.sub('', os.readlink(
                    '/sys/bus/{}/devices/{}'.format(bus, device_bus)))
...
def all_disks(*args, **kwargs):
    ...
    lsblk = json.loads(SysCommand(
        'lsblk --json -l -n -o path,size,type,mountpoint,label,pkname,model').decode('UTF_8'))
...
def get_mount_info(path: Union[pathlib.Path, str], traverse=False, return_real_path=False) -> dict:
    for traversal in list(map(str, [str(path)] + list(pathlib.Path(str(path)).parents))):
        try:
            log(
                f"Getting mount information for device path {traversal}", level=logging.INFO)
            output = SysCommand(
                f'/usr/bin/findmnt --json {traversal}').decode('UTF-8')
            if output:
                break
        except SysCallError:
            pass
    ...
    if not output:
        raise DiskError(
            f"Could not get mount information for device path {path}")

    output = json.loads(output)
    if 'filesystems' in output:
        if len(output['filesystems']) > 1:
            raise DiskError(
                f"Path '{path}' contains multiple mountpoints: {output['filesystems']}")
...
def get_partitions_in_use(mountpoint) -> list:
    ...
    try:
        output = SysCommand(
            f"/usr/bin/findmnt --json -R {mountpoint}").decode('UTF-8')
    except SysCallError:
        return []
    ...
    for target in output.get('filesystems', []):
        mounts.append(Partition(target['source'], None, filesystem=target.get(
            'fstype', None), mountpoint=target['target']))

        for child in target.get('children', []):
            mounts.append(Partition(child['source'], None, filesystem=child.get(
                'fstype', None), mountpoint=child['target']))
...
def disk_layouts():
    try:
        if (handle := SysCommand("lsblk -f -o+TYPE,SIZE -J")).exit_code == 0:
            ...
            log(f"Could not return disk layouts: {handle}",
...
def convert_device_to_uuid(path: str) -> str:
    for i in range(storage['DISK_RETRY_ATTEMPTS']):
        ...
        output = json.loads(SysCommand(
            f"lsblk --json -o+UUID {path}").decode('UTF-8'))
    ...
    raise DiskError(
        f"Could not retrieve the UUID of {path} within a timely manner.")
