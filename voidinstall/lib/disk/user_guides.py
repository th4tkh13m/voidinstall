...
def suggest_single_disk_layout(block_device, default_filesystem=None, advanced_options=False):
    ...
    if default_filesystem == 'btrfs':
        using_subvolumes = input(
            'Would you like to use BTRFS subvolumes with a default structure? (Y/n): ').strip().lower() in ('', 'y', 'yes')
...
def suggest_multi_disk_layout(block_devices, default_filesystem=None, advanced_options=False):
    ...
    MIN_SIZE_TO_ALLOW_HOME_PART = 40  # Gb
    # Gb, rough estimate taking in to account user desktops etc. TODO: Catch user packages to detect size?
    ARCH_LINUX_INSTALLED_SIZE = 20

    block_devices = sort_block_devices_based_on_performance(
        block_devices).keys()

    home_device = select_largest_device(
        block_devices, gigabytes=MIN_SIZE_TO_ALLOW_HOME_PART)
