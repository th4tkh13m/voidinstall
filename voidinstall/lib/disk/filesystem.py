...
class Filesystem:
    ...
    def partuuid_to_index(self, uuid):
        for i in range(storage['DISK_RETRY_ATTEMPTS']):
            self.partprobe()
            output = json.loads(SysCommand(
                f"lsblk --json -o+PARTUUID {self.blockdevice.device}").decode('UTF-8'))
        ...
        for partition in layout.get('partitions', []):
            ...
            if partition.get('format', False) and not partition.get('PARTUUID', None):
                ...
                partition['device_instance'] = self.add_partition(partition.get('type', 'primary'),
                                                                  # TODO: Revisit sane block starts (4MB for memorycards for instance)
                                                                  start=partition.get(
                                                                      'start', '1MiB'),
                                                                  end=partition.get(
                                                                      'size', '100%'),
                ...
                raise ValueError(
                    f"{self}.load_layout() doesn't know how to continue without a new partition definition or a UUID ({partition.get('PARTUUID')}) on the device ({self.blockdevice.get_partition(uuid=partition_uuid)}).")
                ...
                if partition.get('encrypted', False):
                    if not partition.get('!password'):
                        if not storage['arguments'].get('!encryption-password'):
                            if storage['arguments'] == 'silent':
                                raise ValueError(
                                    f"Missing encryption password for {partition['device_instance']}")

                            from ..user_interaction import get_password
                            storage['arguments']['!encryption-password'] = get_password(
                                f"Enter a encryption password for {partition['device_instance']}")
                    ...
                    with luks2(partition['device_instance'], loopdev, partition['!password'], auto_unmount=True) as unlocked_device:
                                ...
                                if not partition['filesystem'].get('format', False):
                                    while True:
                                        partition['filesystem']['format'] = input(
                                            f"Enter a valid fs-type for newly encrypted partition {partition['filesystem']['format']}: ").strip()
                        ...
                        unlocked_device.format(
                            partition['filesystem']['format'], options=partition.get('options', []))
            ...
            if partition.get('boot', False):
                ...
                self.set(self.partuuid_to_index(
                    partition['device_instance'].uuid), 'boot on')
    ...
    def raw_parted(self, string: str):
        if (cmd_handle := SysCommand(f'/usr/bin/parted -s {string}')).exit_code != 0:
            log(f"Parted ended with a bad exit code: {cmd_handle}",
    ...
    def parted(self, string: str):
        ...
        if (parted_handle := self.raw_parted(string)).exit_code == 0:
            ...
            raise DiskError(
                f"Parted failed to add a partition: {parted_handle}")
    ...
    def add_partition(self, partition_type, start, end, partition_format=None):
        ...
        previous_partition_uuids = {
            partition.uuid for partition in self.blockdevice.partitions.values()}

        if self.mode == MBR:
            if len(self.blockdevice.partitions) > 3:
                DiskError(
                    "Too many partitions on disk, MBR disks can only have 3 primary partitions")
        ...
        if self.parted(parted_string):
            ...
            while count < 10:
                new_uuid = None
                new_uuid_set = (previous_partition_uuids ^ {
                                partition.uuid for partition in self.blockdevice.partitions.values()})
                ...
                if new_uuid:
                    ...
                    count += 1
                    log(
                        f"Could not get UUID for partition. Waiting for the {count} time", level=logging.DEBUG)
                    time.sleep(
                        float(storage['arguments'].get('disk-sleep', 0.2)))
            else:
                log("Add partition is exiting due to excessive wait time",
                    level=logging.INFO)
                raise DiskError(
                    f"New partition never showed up after adding new partition on {self}.")
    ...
    def parted_mklabel(self, device: str, disk_label: str):
        log(f"Creating a new partition label on {device}",
