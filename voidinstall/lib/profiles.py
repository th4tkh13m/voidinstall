...
def grab_url_data(path):
    safe_path = path[: path.find(':') + 1] + ''.join([item if item in ('/', '?', '=', '&') else urllib.parse.quote(item)
        ...
        for root, folders, files in os.walk(os.path.abspath(os.path.expanduser(PATH_ITEM + subpath))):
            for file in files:
                ...
                if os.path.splitext(file)[1] == '.py':
                    ...
                    cache[file[:-3]] = {'path': os.path.join(
                        root, file), 'description': description, 'tailored': tailored}
            break

    # Grab profiles from upstream URL
    if storage['PROFILE_DB']:
        profiles_url = os.path.join(
            storage["UPSTREAM_URL"] + subpath, storage['PROFILE_DB'])
        try:
            profile_list = json.loads(grab_url_data(profiles_url))
        except urllib.error.HTTPError as err:
            print(
                f'Error: Listing profiles on URL "{profiles_url}" resulted in:', err)
            return cache
        except json.decoder.JSONDecodeError as err:
            print(
                f'Error: Could not decode "{profiles_url}" result as JSON:', err)
            return cache
        ...
        for profile in profile_list:
            if os.path.splitext(profile)[1] == '.py':
                ...
                cache[profile[:-3]] = {'path': os.path.join(
                    storage["UPSTREAM_URL"] + subpath, profile), 'description': profile_list[profile], 'tailored': tailored}
        ...
        if (url := urllib.parse.urlparse(profile_path)).scheme and url.scheme in ('https', 'http'):
            if not self.converted_path:
                ...
                with open(self.converted_path, "w") as temp_file:
                    temp_file.write(urllib.request.urlopen(
                        url.geturl()).read().decode('utf-8'))
...
class Application(Profile):
    ...
    def path(self):
        ...
        if not parsed_url.scheme:
            ...
            raise ProfileNotFound(
                f"Application cannot handle scheme {parsed_url.scheme}")
