...
class Journald(dict):
    ...
    def log(message, level=logging.DEBUG):
        ...
        if level == LogLevels.Critical:
            log("Deprecated level detected in log message, please use new logging.<level> instead for the following log message:",
                fg="red", level=logging.ERROR, force=True)
...
def stylize_output(text: str, *opts, **kwargs):
    opt_dict = {'bold': '1', 'italic': '3', 'underscore': '4',
...
def log(*args, **kwargs):
    ...
    if filename := storage.get('LOG_FILE', None):
        ...
        try:
            Path(absolute_logfile).parents[0].mkdir(
