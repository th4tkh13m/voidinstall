...
class Installer:
    ...
    def __init__(self, target, *, base_packages=None, kernels=None):
        ...
        self.BINARIES = []
        self.FILES = []
        self.HOOKS = ["base", "udev", "autodetect", "keyboard",
    ...
    def __exit__(self, *args, **kwargs):
        ...
        if not (missing_steps := self.post_install_check()):
            self.log('Installation completed without any errors. You may now reboot.',
                     fg='green', level=logging.INFO)
            self.sync_log_to_install_medium()

            return True
        else:
            self.log('Some required steps were not successfully installed/configured before leaving the installer:',
                     fg='red', level=logging.WARNING)
            for step in missing_steps:
                self.log(f' - {step}', fg='red', level=logging.WARNING)

            self.log(
                f"Detailed error logs can be found at: {storage['LOG_PATH']}", level=logging.WARNING)
            self.log(
                "Submit this zip file as an issue to https://github.com/archlinux/archinstall/issues", level=logging.WARNING)
    ...
    def mount_ordered_layout(self, layouts: dict):
        ...
        for mountpoint in sorted(mountpoints.keys()):
            ...
            if partition.get('encrypted', False):
                ...
                if not (password := partition.get('!password', None)):
                    raise RequirementError(
                        f"Missing mountpoint {mountpoint} encryption password in layout: {partition}")
                ...
                with (luks_handle := luks2(partition['device_instance'], loopdev, password, auto_unmount=False)) as unlocked_device:
                    if partition.get('generate-encryption-key-file'):
                        ...
                        luks_handle.add_key(pathlib.Path(
                            f"{self.target}{encryption_key_path}"), password=password)
                        luks_handle.crypttab(self, encryption_key_path, options=[
                                             "luks", "key-slot=1"])

                    log(f"Mounting {mountpoint} to {self.target}{mountpoint} using {unlocked_device}",
                        level=logging.INFO)
                    unlocked_device.mount(f"{self.target}{mountpoint}")

            else:
                log(
                    f"Mounting {mountpoint} to {self.target}{mountpoint} using {partition['device_instance']}", level=logging.INFO)
                partition['device_instance'].mount(
            ...
            try:
                ...
                raise DiskError(
                    f"Target {self.target}{mountpoint} never got mounted properly (unable to get mount information using findmnt).")
    ...
    def pacstrap(self, *packages, **kwargs):
        ...
        if (sync_mirrors := SysCommand('/usr/bin/pacman -Syy')).exit_code == 0:
            if (pacstrap := SysCommand(f'/usr/bin/pacstrap {self.target} {" ".join(packages)} --noconfirm', peak_output=True)).exit_code == 0:
                ...
                self.log(
                    f'Could not strap in packages: {pacstrap}', level=logging.ERROR, fg="red")
                self.log(
                    f'Could not strap in packages: {pacstrap.exit_code}', level=logging.ERROR, fg="red")
                raise RequirementError(
                    "Pacstrap failed. See /var/log/archinstall/install.log or above message for error details.")
        else:
            self.log(
                f'Could not sync mirrors: {sync_mirrors.exit_code}', level=logging.INFO)
    ...
    def genfstab(self, flags='-pU'):
        ...
        with open(f"{self.target}/etc/fstab", 'a') as fstab_fh:
            fstab_fh.write((fstab := SysCommand(
                f'/usr/bin/genfstab {flags} {self.target}')).decode())
    ...
    def activate_ntp(self):
        log(f"activate_ntp() is deprecated, use activate_time_syncronization()",
            fg="yellow", level=logging.INFO)
    ...
    def enable_espeakup(self):
        self.log(
            'Enabling espeakup.service for speech synthesis (accessibility).', level=logging.INFO)
    ...
    def minimal_installation(self):
        ...
        for partition in self.partitions:
            ...
            if self.detect_encryption(partition):
                if 'encrypt' not in self.HOOKS:
                    self.HOOKS.insert(self.HOOKS.index(
                        'filesystems'), 'encrypt')
        ...
        if not is_vm():
            ...
            if vendor == "AuthenticAMD":
                ...
                self.log(
                    f"Unknown CPU vendor '{vendor}' detected. Archinstall won't install any ucode.", level=logging.DEBUG)
        ...
        for function in self.post_base_install:
            self.log(
                f"Running post-installation hook: {function}", level=logging.INFO)
            function(self)
    ...
    def setup_swap(self, kind='zram'):
        if kind == 'zram':
            ...
            raise ValueError(
                f"Archinstall currently only supports setting up swap on zram")
    ...
    def add_bootloader(self, bootloader='systemd-bootctl'):
        ...
        if boot_partition is None and root_partition is None:
            raise ValueError(
                f"Could not detect root (/) or boot (/boot) in {self.target} based on: {self.partitions}")

        self.log(
            f'Adding bootloader {bootloader} to {boot_partition if boot_partition else root_partition}', level=logging.INFO)
        ...
        if bootloader == 'systemd-bootctl':
            ...
            with open(f'{self.target}/boot/loader/loader.conf', 'w') as loader:
                for line in loader_data:
                    if line[:8] == 'default ':
                        loader.write(
                            f'default {self.init_time}_{self.kernels[0]}\n')
                ...
                with open(f'{self.target}/boot/loader/entries/{self.init_time}_{kernel}.conf', 'w') as entry:
                    ...
                    if not is_vm():
                        ...
                        if vendor == "AuthenticAMD":
                            ...
                            self.log(
                                "unknow cpu vendor, not adding ucode to systemd-boot config")
                    ...
                    if real_device := self.detect_encryption(root_partition):
                        ...
                        log(
                            f"Identifying root partition by PART-UUID on {real_device}: '{real_device.uuid}'.", level=logging.DEBUG)
                        entry.write(
                            f'options cryptdevice=PARTUUID={real_device.uuid}:luksdev root=/dev/mapper/luksdev rw intel_pstate=no_hwp rootfstype={root_fs_type} {" ".join(self.KERNEL_PARAMS)}\n')
                    else:
                        log(
                            f"Identifying root partition by PART-UUID on {root_partition}, looking for '{root_partition.uuid}'.", level=logging.DEBUG)
                        entry.write(
                            f'options root=PARTUUID={root_partition.uuid} rw intel_pstate=no_hwp rootfstype={root_fs_type} {" ".join(self.KERNEL_PARAMS)}\n')
            ...
            if real_device := self.detect_encryption(root_partition):
                root_uuid = SysCommand(
                    f"blkid -s UUID -o value {real_device.path}").decode().rstrip()
                ...
                log(f"Using UUID {root_uuid} of {real_device} as encrypted root identifier.",
                    level=logging.INFO)
                SysCommand(
                    f"/usr/bin/arch-chroot {self.target} {add_to_CMDLINE_LINUX} {_file}")
                SysCommand(
                    f"/usr/bin/arch-chroot {self.target} {enable_CRYPTODISK} {_file}")
            else:
                _file = "/etc/default/grub"
            ...
            log(f"GRUB uses {boot_partition.path} as the boot partition.",
                level=logging.INFO)
            if has_uefi():
                # TODO: Do we need? Yes, but remove from minimal_installation() instead?
                self.pacstrap('efibootmgr')
                if not (handle := SysCommand(f'/usr/bin/arch-chroot {self.target} grub-install --target=x86_64-efi --efi-directory=/boot --bootloader-id=GRUB')).exit_code == 0:
                    raise DiskError(
                        f"Could not install GRUB to {self.target}/boot: {handle}")
            else:
                if not (handle := SysCommand(f'/usr/bin/arch-chroot {self.target} grub-install --target=i386-pc --recheck {boot_partition.parent}')).exit_code == 0:
                    raise DiskError(
                        f"Could not install GRUB to {boot_partition.path}: {handle}")
            ...
            for kernel in self.kernels:
                ...
                if not is_vm():
                    ...
                    if vendor == "AuthenticAMD":
                        ...
                        self.log(
                            "unknow cpu vendor, not adding ucode to firmware boot entry")
                ...
                SysCommand(
                    f'efibootmgr --disk {boot_partition.path[:-1]} --part {boot_partition.path[-1]} --create --label "{label}" --loader {loader} --unicode \'{" ".join(kernel_parameters)}\' --verbose')

            self.helper_flags['bootloader'] = bootloader
        else:
            raise RequirementError(
                f"Unknown (or not yet implemented) bootloader requested: {bootloader}")
    ...
    def enable_sudo(self, entity: str, group=False):
        self.log(
            f'Enabling sudo permissions for {entity}.', level=logging.INFO)
    ...
    def set_keyboard_language(self, language: str) -> bool:
        ...
        if len(language.strip()):
            if not verify_keyboard_layout(language):
                self.log(
                    f"Invalid keyboard language specified: {language}", fg="red", level=logging.ERROR)
                return False
            ...
            with Boot(self) as session:
                ...
                if (output := session.SysCommand(["localectl", "set-keymap", language])).exit_code != 0:
                    raise ServiceException(
                        f"Unable to set locale '{language}' for console: {output}")

                self.log(
                    f"Keyboard language for this installation is now set to: {language}")
        else:
    ...
    def set_x11_keyboard_language(self, language: str) -> bool:
        ...
        if len(language.strip()):
            if not verify_x11_keyboard_layout(language):
                self.log(
                    f"Invalid x11-keyboard language specified: {language}", fg="red", level=logging.ERROR)
                return False
            ...
            with Boot(self) as session:
                ...
                if (output := session.SysCommand(["localectl", "set-x11-keymap", language])).exit_code != 0:
                    raise ServiceException(
                        f"Unable to set locale '{language}' for X11: {output}")
        else:
