...
class luks2:
    ...
    def __enter__(self):
        if not self.key_file:
            # TODO: Make disk-pw-file randomly unique?
            self.key_file = f"/tmp/{os.path.basename(self.partition.path)}.disk_pw"
    ...
    def encrypt(self, partition, password=None, key_size=512, hash_type='sha512', iter_time=10000, key_file=None):
        log(f'Encrypting {partition} (This might take a while)',
            level=logging.INFO)
        ...
        if not key_file:
            if self.key_file:
                ...
                # TODO: Make disk-pw-file randomly unique?
                key_file = f"/tmp/{os.path.basename(self.partition.path)}.disk_pw"
        ...
        try:
            ...
            if cmd_handle.exit_code != 0:
                raise DiskError(
                    f'Could not encrypt volume "{partition.path}": {b"".join(cmd_handle)}')
            ...
            if err.exit_code == 256:
                ...
                devinfo = json.loads(b''.join(SysCommand(
                    f"lsblk --fs -J {partition.path}")).decode('UTF-8'))['blockdevices'][0]
                ...
                if len(children := devinfo.get('children', [])):
                    for child in children:
                        ...
                        if child_mountpoint := child.get('mountpoint', None):
                            log(f'Unmounting {child_mountpoint}',
                                level=logging.DEBUG)
                            SysCommand(f"umount -R {child_mountpoint}")

                        # And close it if possible.
                        log(f"Closing crypt device {child['name']}",
                            level=logging.DEBUG)
    ...
    def unlock(self, partition, mountpoint, key_file):
        ...
        if os.path.islink(f'/dev/mapper/{mountpoint}'):
            ...
            unlocked_partition = Partition(self.mapdev, None, encrypted=True, filesystem=get_filesystem_type(
                self.mapdev), autodetect_filesystem=False)
    ...
    def add_key(self, path: pathlib.Path, password: str):
        if not path.exists():
            raise OSError(
                2, f"Could not import {path} as a disk encryption key, file is missing.", str(path))

        log(f'Adding additional key-file {path} for {self.partition}',
            level=logging.INFO)

        worker = SysCommandWorker(
            f"/usr/bin/cryptsetup -q -v luksAddKey {self.partition.path} {path}")
        pw_injected = False
        ...
        if worker.exit_code != 0:
            raise DiskError(
                f'Could not add encryption key {path} to {self.partition} because: {worker}')

    def crypttab(self, installation, key_path: str, options=["luks", "key-slot=1"]):
        log(
            f'Adding a crypttab entry for key {key_path} in {installation}', level=logging.INFO)
