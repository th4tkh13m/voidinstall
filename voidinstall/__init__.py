...
def initialize_arguments():
    ...
    parser.add_argument("--config", nargs="?",
                        help="JSON configuration file or URL")
    ...
    parser.add_argument("--script", default="guided", nargs="?",
                        help="Script to run for installation", type=str)
    ...
    if args.config is not None:
        try:
            ...
            # The Profile was not a direct match on a remote URL, it must be a local file.
...
if arguments.get('debug'):
    log(
        f"Warning: --debug mode will write certain credentials to {storage['LOG_PATH']}/{storage['LOG_FILE']}!", fg="red", level=logging.WARNING)
...
def run_as_a_module():
    ...
    storage['PROFILE_PATH'].append(os.path.abspath(
        f'{os.path.dirname(__file__)}/examples'))
    try:
