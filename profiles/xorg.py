            ...
            if "linux-zen" in archinstall.storage['installation_session'].base_packages or "linux-lts" in archinstall.storage['installation_session'].base_packages:
                for kernel in archinstall.storage['installation_session'].kernels:
                    archinstall.storage['installation_session'].add_additional_packages(
                        f"{kernel}-headers")  # Fixes https://github.com/archlinux/archinstall/issues/585
                # I've had kernel regen fail if it wasn't installed before nvidia-dkms
                archinstall.storage['installation_session'].add_additional_packages(
                    "dkms")
                archinstall.storage['installation_session'].add_additional_packages(
                    "xorg-minimal xorg-xinit nvidia-dkms")
            else:
                archinstall.storage['installation_session'].add_additional_packages(
                    f"xorg-minimal xorg-xinit {' '.join(archinstall.storage.get('gfx_driver_packages', []))}")
        else:
            archinstall.storage['installation_session'].add_additional_packages(
                f"xorg-minimal xorg-xinit {' '.join(archinstall.storage.get('gfx_driver_packages', []))}")
    except Exception as err:
        archinstall.log(
            f"Could not handle nvidia and linuz-zen specific situations during xorg installation: {err}", level=logging.WARNING, fg="yellow")
        archinstall.storage['installation_session'].add_additional_packages(
            "xorg-minimal xorg-xinit")  # Prep didn't run, so there's no driver to install
